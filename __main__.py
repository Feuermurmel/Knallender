...
def parse_args():
    ...
    if args.start_week is None:
        if args.start_year is not None:
            parser.error(
                'Either none or both a start year and week need to be '
                'specified.')
        
        args.start_year, args.start_week, _ = \
...
month_names = 'Januar Februar März April Mäi Juni Juli Auguscht Septämber ' \
              'Oktober Novämber Dezämber'.split()
...
def main(start_year, start_week, weeks_per_page, pages, cell_size, paper_size, debug):
    for page in range(pages):
        first_week = datetime_from_iso_week(start_year, start_week) \
                     + datetime.timedelta(weeks=page * weeks_per_page)
            ...
            with open(asy_path, 'w', encoding='utf-8') as file:
                ...
                def label(font, size, alignment, text, position_expression, *args):
                    write(
                        ...
                        font,
                        text,
                        position_expression.format(*args),
                        alignment,
                        size)
                ...
                write(
                    'pair raster(real x, real y) {{ return (paper_size - '
                    '(cell_size.x * 7 - header_width, cell_size.y * '
                    'weeks_per_page)) / 2 + (cell_size.x * x, cell_size.y * '
                    'y); }}')
                
                for i in range(1, weeks_per_page):
                    write(
                        'draw(raster(0, {0}) -- raster(7, {0}), '
                        'cell_border_pen);',
                        i)
                
                for i in range(1, 7):
                    write(
                        'draw(raster({0}, 0) -- raster({0}, weeks_per_page), '
                        'cell_border_pen);',
                        i)
                ...
                write(
                    'draw(box(raster(0, 0), raster(7, weeks_per_page)), '
                    'cell_border_pen);')
